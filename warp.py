...
'''def generate_offset_map(source, target):
    ...
    for i in range(source.shape[0]):
        _source = np.concatenate([source[i,:,:], anchor_pts], axis=0).astype(np.float32)
        _target = np.concatenate([target[i,:,:], anchor_pts], axis=0).astype(np.float32)
        ...
        _offsetmap = np.stack([_offsetmapy, _offsetmapx, _offsetmapx*0], axis=2)
        offsetmap.append(_offsetmap)
    offsetmap = np.stack(offsetmap, axis=0)
    return offsetmap'''
...
def generate_uv_map(source, uv, img_size):
    ...
    #_interpx = mtri.CubicTriInterpolator(_triang, uv[:,0])
    #_interpy = mtri.CubicTriInterpolator(_triang, uv[:,1])
    #_interpz = mtri.CubicTriInterpolator(_triang, uv[:,2])
    _interpx = mtri.LinearTriInterpolator(_triang, uv[:,0])
    _interpy = mtri.LinearTriInterpolator(_triang, uv[:,1])
    _interpz = mtri.LinearTriInterpolator(_triang, uv[:,2])
    _offsetmapx = _interpx(xi, yi)
    _offsetmapy = _interpy(xi, yi)
    _offsetmapz = _interpz(xi, yi)

    offsetmap = np.stack([_offsetmapy, _offsetmapx, _offsetmapz], axis=2)
